...
class TextFrontend:
    """Pinyin frontend (matches the vendor demo.py exactly).

    Chinese -> tone-marked pinyin syllables (pypinyin TONE3 + neutral tone),
    English/punctuation -> SentencePiece BPE; ids come from tokens.txt
    (line number = token id). Arabic numbers are converted to Chinese numerals
    before pinyin conversion.
    """
    ...
    def __init__(self, tokens_txt: str, bpe_model: str, max_tokens: int = 48):
        import sentencepiece as spm

        with open(tokens_txt, encoding="utf-8") as f:
            surfaces = [line.rstrip("\n") for line in f]
        if surfaces and surfaces[-1] == "<pad>":
            surfaces = surfaces[:-1]
        self.id_of_surface = {s: i for i, s in enumerate(surfaces)}
        self.sp = spm.SentencePieceProcessor()
        self.sp.Load(bpe_model)
        self.max_tokens = max_tokens

    def _normalize(self, text: str) -> str:
        text = numbers_to_zh(text)
        text = text.translate(str.maketrans(self._VENDOR_PUNCT))
    ...
    def _bpe_surfaces(self, seg: str) -> list[str]:
        seg = seg.strip()
        if not seg:
            return []
        ids = self.sp.EncodeAsIds("A " + seg)[1:]
        return [self.sp.IdToPiece(i) for i in ids if i not in (self.UNK_ID, self.SPACE_MARKER_ID)]
    ...
    def tokenize(self, text: str) -> list[str]:
        ...
        for seg in re.split(r"([\u4e00-\u9fff]+)", text):
            ...
            if re.fullmatch(r"[\u4e00-\u9fff]+", seg):
                out.extend(x[0] for x in pinyin(
                    seg, style=Style.TONE3, neutral_tone_with_five=True))
            else:
                out.extend(self._bpe_surfaces(seg))
        return out

    def text2ids(self, text: str) -> list[int]:
        return [self.id_of_surface[s] for s in self.tokenize(text)
                if s in self.id_of_surface]
...
class PocketTTS:
    ...
    def stream(self, token_ids: list[int], ref_audio: np.ndarray, temp: float = 0.0,
        ...
        for frame in range(max_frames):
            ...
            t_stage = time.perf_counter()
            flow_feed = {"conditioning": conditioning, "noise": noise}
            if any(name == "decode_steps" for name, _, _ in self.flow_net.inputs):
                flow_feed["decode_steps"] = np.asarray(
                    float(cfg["decode_steps"]), dtype=np.float32)
            flow_out = self.flow_net.run(cast_feed(self.flow_net, flow_feed))
...
def main() -> None:
    ...
    ap.add_argument("--tokens-txt", required=True, help="vocab file (line number = token id)")
    ap.add_argument("--bpe-model", required=True, help="SentencePiece model for English/punct")
    ...
    frontend = TextFrontend(args.tokens_txt, args.bpe_model)
