...
struct TtsSession {
    ...
    void load(const std::string& path, int threads, int spin,
...
struct WavWriter {
    ...
    void write(const float* audio, size_t n, bool flush = true) {
...
struct Options {
    ...
    std::string output;
    std::string flow_ar_model = "flow_ar_step_int8.onnx";
    std::string flow_prefill_model = "flow_step_int8.onnx";
    int threads = 4;
...
int main(int argc, char** argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        else if (a == "--flow-ar-model") opt.flow_ar_model = next();
        else if (a == "--flow-prefill-model") opt.flow_prefill_model = next();
        ...
        else if (a == "--spin") opt.spin = std::stoi(next());
        else if (a == "--profile") opt.profile_prefix = next();
    ...
    flow.load(md + "/" + opt.flow_prefill_model, opt.prefill_threads, 1);
    flow_ar.load(md + "/" + opt.flow_ar_model, opt.threads, opt.spin,
                 opt.profile_prefix.empty() ? "" : opt.profile_prefix + "_flow_ar");
    mimi_tf.load(md + "/mimi_split/mimi_transformer_step_int8.onnx",
                 opt.mimi_threads > 0 ? opt.mimi_threads : opt.threads, opt.spin,
                 opt.profile_prefix.empty() ? "" : opt.profile_prefix + "_mimi_tf");
    ...
    struct StageTiming {
        std::atomic<double> ar_ms{0}, flow_net_ms{0}, mimi_tf_ms{0}, mimi_conv_ms{0}, wait_ms{0};
        std::atomic<long> ar_calls{0}, mimi_calls{0};
    } timing;
    ...
    for (size_t ci = 0; ci < chunks.size(); ++ci) {
        ...
        auto run_mimi = [&](std::vector<float> input_latent, bool emit) -> MimiResult {
            ...
            timing.mimi_tf_ms.store(timing.mimi_tf_ms.load() + ms_since(t_tf));
            timing.mimi_calls.fetch_add(1);
            ...
            mimi_kv.append(tf_out[1].GetTensorData<float>(), kStepsPerLatent);
            {
                ...
                timing.mimi_conv_ms.store(timing.mimi_conv_ms.load() + ms_since(t_cv));
            }
        ...
        auto ar_flownet = [&](const float* in_latent, std::vector<float>& out_latent) -> float {
            ...
            feed.push_back(std::move(kv_t));
            feed.push_back(std::move(off_t));
            const auto t_ar = Clock::now();
            auto out = flow_ar.run(feed);
            timing.ar_ms.store(timing.ar_ms.load() + ms_since(t_ar));
            timing.ar_calls.fetch_add(1);
            ...
            flow_kv.append(out[2].GetTensorData<float>(), 1);
            {
                ...
                timing.flow_net_ms.store(timing.flow_net_ms.load() + ms_since(t_fn));
            }
            return eos_logit;
        };
        ...
        for (int frame = 1; !stopped && frame < opt.max_frames; ++frame) {
            ...
            timing.wait_ms.store(timing.wait_ms.load() + ms_since(t_wait));
            have_pending = false;
            if (res.emit) {
                if (first_frame_ms < 0) first_frame_ms = ms_since(chunk_start);
                frames_total += 1;
                samples_total += kFrameSize;
                writer.write(res.audio.data(), kFrameSize, opt.streaming != 0);
            }
        ...
        if (ci + 1 < chunks.size() && pause_samples > 0) {
            writer.write(silence.data(), silence.size(), opt.streaming != 0);
    ...
    {
        const long n_ar = std::max<long>(timing.ar_calls.load(), 1);
        const long n_mm = std::max<long>(timing.mimi_calls.load(), 1);
        std::printf("stage: AR=%.2fms flow_net=%.2fms mimi_tf=%.2fms mimi_conv=%.2fms "
                    "wait=%.2fms/frame total_wall=%.2fms/frame\n",
                    timing.ar_ms.load() / n_ar, timing.flow_net_ms.load() / n_ar,
                    timing.mimi_tf_ms.load() / n_mm, timing.mimi_conv_ms.load() / n_mm,
                    timing.wait_ms.load() / n_ar,
                    total_ms / std::max<long>(frames_total, 1));
    }
    AX_ENGINE_Deinit();
    AX_SYS_Deinit();
    return 0;
