...
ONNXRUNTIME_DIR="${ONNXRUNTIME_DIR:-$(cd "${ROOT}" && pwd)/.work_tmp/ort/onnxruntime-linux-aarch64-1.26.0}"
...
ORT_LIB="${ONNXRUNTIME_DIR}/lib/libonnxruntime.so.1.26.0"
if [[ -f "${ORT_LIB}" ]]; then
  cp -a "${ORT_LIB}" "${CPP_DIR}/bin/"
  ln -sfn "libonnxruntime.so.1.26.0" "${CPP_DIR}/bin/libonnxruntime.so.1"
fi
